...
* SQL its a lanaguage called Structure Query Language, is the more rigid, structured way of storing data
* Ralational Database have a table-based data structure, with a strict predefined schema required.

2. Why do tables need a `primary key`?
* is a table column that serves a special purpose and it ensures row-level accessibility, the values that compose a primary key column are unique, no two values are the same.

3. What is the name given to a table column that references the primary key on another table.
* A Foreign key

4. What do we need in order to have a _many to many_ relationship between two tables.
* when we have multiple records in a table are associated with multiple records in another table
